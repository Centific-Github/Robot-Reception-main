...
export class VisithistoryIndexCtrl extends BaseCtrl {
    ...
    async pageChanged(): Promise<void> {
        ...
        try {
            let query = scope.query;
            let webVisitHistoryClient: webapi.WebVisitHistoryClient = this.api.getClient(webapi.WebVisitHistoryClient);

            let historySearchVM: webapi.HistorySearchVM = this.api.getClient(webapi.HistorySearchVM);

            historySearchVM.page = scope.pageIndex;
            historySearchVM.size = scope.pageSize;
            historySearchVM.startTime = query.startTime;
            historySearchVM.endTime = query.endTime;

            let dataSource = await webVisitHistoryClient.searchHistoryPage(historySearchVM);
